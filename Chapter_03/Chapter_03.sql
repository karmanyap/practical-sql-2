...
-- Note you can also specify the sort column by using a number representing its position in the result
...
SELECT last_name, school, hire_date
FROM teachers
WHERE school LIKE '%Middle School'
ORDER BY hire_date;
...
-- Teachers earning 43500 or moew

SELECT first_name, last_name, salary
FROM teachers
WHERE salary > 43500;

-- Teachers who earn from $40,000 and $65,000/ Order by hire date to understand correlation between hire date and salary
...
-- Alter method for using WHERE clause conditions
...
-- Listing 3.7: Filtering with LIKE and ILIKE. ILIKE does not seem to function with MSSQL
...
-- Note how the results change if we omit the parentheses. That's because the AND operator takes precendence over OR and is evaluated first.
